...
public class Server5 {
    ...
    public static void main(String args[]) {
        ...
        ServerLogger serverLogger = new ServerLogger("Server5");

        try {
            HashMap<String, Integer> serverPortMap = ServerProperties.getPortMap();

            PaxosServant server = new PaxosServant(5);
            KVInterface stub = (KVInterface) UnicastRemoteObject.exportObject(server, 0);
            Registry registry = LocateRegistry.createRegistry(serverPortMap.get("Server5"));
            registry.bind("utils.KVInterface", stub);

            serverLogger.debug("Server5 is listening at ip: " + InetAddress.getLocalHost() +
                    ", port: " + serverPortMap.get("Server5") + " ...");
